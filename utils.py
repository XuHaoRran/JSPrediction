...
def generate_segmentations_monai(data_loader, model, writer_1, args):
    ...
    for idx, val_data in enumerate(data_loader):
        ...
        segs = torch.zeros((1, 1, ref_seg.shape[0], ref_seg.shape[1], ref_seg.shape[2]))
        ...
        refmap_tumor = ref_seg == 1
        refmap_node = ref_seg == 2
        refmap = np.stack([refmap_tumor, refmap_node])
